"""
Tests for the second curated B1/B2 pack (b1_b2_pack2.json) — high-frequency
everyday words, phrasal verbs and idioms for intermediate learners.
"""
...
PACK = Path("backend/data/b1_b2_pack2.json")


def test_pack2_quality_and_unique():
    pack = json.loads(PACK.read_text(encoding="utf-8"))
    others = set()
    for f in ("core3000_words.json", "b2_extra_words.json", "b1_b2_pack.json"):
    ...
    for w in pack:
        ...
        seen.add(k)
    # plenty of high-frequency phrasal verbs / idioms
    kinds = {w["pos"] for w in pack}
    assert "phrasal verb" in kinds and "idiom" in kinds


def test_pack2_seeds_idioms_and_idempotent():
    async def run():
