"""
Tests for the third curated B1/B2 pack (b1_b2_pack3.json) — more high-frequency
everyday words, phrasal verbs, idioms and connective adverbs for intermediate
learners.
"""
...
PACK = Path("backend/data/b1_b2_pack3.json")


def test_pack3_quality_and_unique():
    pack = json.loads(PACK.read_text(encoding="utf-8"))
    others = set()
    for f in ("core3000_words.json", "b2_extra_words.json",
              "b1_b2_pack.json", "b1_b2_pack2.json"):
...
def test_pack3_seeds_and_idempotent():
    async def run():
